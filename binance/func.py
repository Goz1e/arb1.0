...
# returns tickers json from API
def get_tickers(url):
    return requests.get(url).json()


#turns returned json to a list of pairs 
def get_coin_list(array):
    return [pair['symbol'] for pair in array]
...
# get current prices
def get_prices_for_tpair(t_pair,prices_dict):  
    ...
    for item in prices_dict:
        if item['symbol']==pair_a['symbol']:
            pair_a['ask']=item['askPrice']
            pair_a['bid']=item['bidPrice']
            pair_a['base_amount']=1
            pair_a['quote_amount']=1
        
        if item['symbol']==pair_b['symbol']:
            pair_b['ask']=item['askPrice']
            pair_b['bid']=item['bidPrice']
            pair_b['base_amount']=1
            pair_b['quote_amount']=1
        
        if item['symbol']==pair_c['symbol']:
            pair_c['ask']=item['askPrice']
            pair_c['bid']=item['bidPrice']
            pair_c['base_amount']=1
            pair_c['quote_amount']=1


# a convinence method to find the next pair to be traded
...
def cal_surface_rate(tri_pair):
    ...
    for direction in directions:
        ...
        if direction == 'foward': #Base to Quote swap
            t1_rate = 1/float(pair_a_ask) if float(pair_a_ask) != 0 else 0
        ...
        t1_msg = f'1: swap {t1_direction} @ rate:{t1_rate} => {t1_acquired_coin}'
        ...
        if coin == next_pair_base: #foward swap (Base to Quote)
            t2_rate = 1/float(next_pair_ask) if float(next_pair_ask) != 0 else 0
        ...
        t2_msg = f'2: swap {t2_direction} @ rate:{t2_rate} => {t2_acquired_coin}'
        ...
        t3_msg = f'3: swap {t3_direction} @ rate:{t3_rate} => {t3_acquired_coin}'
        # t3_msg = f'3: swap {t3_direction} @ rate:{t3_rate} => {t3_acquired_coin} in {direction}'
        
        profit = t3_acquired_coin - initial_amount
        percentage = (profit/initial_amount) * 100
        if profit > 0 and percentage < 20:
            # output result 
            surface_dict = {
                'swap_1': coin,
                'swap_2': t2_coin,
...
# Reformat orderbook for dept calculation
...
# convience  - get Acquired coin for dept calculation
...
"""WE START HERE"""
# get the dept fromthe order book
def get_dept_from_order_book(surface_arb):
    
    # Extract initial variables
    swap_1 = surface_arb['swap_1'] #assumed to be the starting coin (capital)
    starting_amount = 100
    ...
    # define direction and description of trades
